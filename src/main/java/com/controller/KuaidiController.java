    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(kuaidi !=null){
            ...
            BeanUtils.copyProperties( kuaidi , view );//把实体数据重构到view中
                ...
                //级联表
                XueshengEntity xuesheng = xueshengService.selectById(kuaidi.getXueshengId());
                if(xuesheng != null){
    ...
    */
    ...
    public R save(@RequestBody KuaidiEntity kuaidi, HttpServletRequest request){
        ...
        if(StringUtil.isEmpty(role))
            ...
            kuaidi.setXueshengId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<KuaidiEntity> queryWrapper = new EntityWrapper<KuaidiEntity>()
            .eq("xuesheng_id", kuaidi.getXueshengId())
            .eq("laifang_name", kuaidi.getLaifangName())
            .eq("kuaidi_types", kuaidi.getKuaidiTypes())
            .eq("shifou_types", kuaidi.getShifouTypes())
            ;
...
//                            kuaidiEntity.setXueshengId(Integer.valueOf(data.get(0)));   //学生 要改的
//                            kuaidiEntity.setLaifangName(data.get(0));                    //快递单号 要改的
//                            kuaidiEntity.setKuaidiTypes(Integer.valueOf(data.get(0)));   //快递公司 要改的
//                            kuaidiEntity.setShifouTypes(Integer.valueOf(data.get(0)));   //是否领取 要改的
